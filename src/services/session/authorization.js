...
const USER_KEY = 'referral/USER_KEY';
const REFERER_LINK_KEY = 'referral/LINK';
const TASK_ID_KEY = 'referral/TASK_ID';
...
const customTasksPath = '/custom_tasks/';
const customTaskStatusesPath = '/custom_task_statuses/';
...
const appConfigPath = '/constances/';
...
export function saveReferralInfo(link, task) {
    var token = jwt.sign({
        auth_token: 'Token ' + loadToken(),
        api_url: API_URL + 'v0/tasks/' + task + '/interact/',
...
export function getCustomTasks(data) {
    return new Promise(resolve =>
        get(customTasksPath, data).then((resp) => {
            resolve(resp.data); 
        })
        .catch(err => resolve(false))
    );
...
export function getCustomTaskStatuses() {
    return new Promise(resolve => 
        get(customTaskStatusesPath).then((resp) => {
            resolve(resp.data);
        })
        .catch(err => resolve(false))
    );
...
export function updateCustomTask(data) {
    const path = `${customTasksPath}${data.id}/`;
    return new Promise(resolve => {
        put(path, data).then((resp) => {
            if (!resp.error) {
                showSuccess('Success', 'Task has been successfully updated');
...
export function interactCustomTask(id) {
    const url = `${customTasksPath}${id}/interact/`;
    return new Promise(resolve =>
        post(url).then((resp) => {
            if (!resp.error) {
                showSuccess('Success', 'Task Have been successfully Updated!');
...
export function loadAppConfig() {
    return new Promise(resolve =>
        get(appConfigPath).then((resp) => {
            if (!resp.error) {
                resp.data.map((con, id) => localStorage.setItem(con.key, con.value));
            }
            resolve(resp.data);
        })
        .catch(err => {
...
export function loadAppConfigItem(KEY) {
