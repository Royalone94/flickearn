...
import { getWallets, postTransferFund, loadAppConfigItem } from '../../../services/session/authorization';
...
class TransferFunds extends Component {
    ...
    checkAmount = (rule, value, callback) => {
        ...
        var minAmount = parseInt(loadAppConfigItem('MINIMUM_TRANSFER_AMOUNT'));

        if(amount<minAmount)
            callback(`Minimum amount to transfer: $${minAmount}.`);
        else {
            callback();
        }
    }
...
const TransferForm = Form.create()(MixpanelTrackWrapper(TransferFunds, 'TransferFunds'));
