...
import { getWallets, createWithdraw, getWithdrawPaymentMethods, loadAppConfigItem } from '../../../services/session/authorization';
...
class Withdraw extends Component {
    ...
    checkAmount = (rule, value, callback) => {
        ...
        var minAmount = parseInt(loadAppConfigItem('MINIMUM_WITHDRAWAL_AMOUNT'));

        if(this.sender !=this.state.referral_wallet_id) {
            if(amount<minAmount)
                callback(`Minimum amount to withdraw: $${minAmount}.`);
            else {
                callback();
            }
        }  else {
...
const WithdrawForm = Form.create()(MixpanelTrackWrapper(Withdraw, 'Withdraw'));
